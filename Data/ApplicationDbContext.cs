...
public class ApplicationDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Bingo>().HasData(new Bingo { Id = 1, Name = "Bingo 1" });
        ...
        modelBuilder.Entity<Player>().HasOne(p => p.CurrentBingoGame);

        modelBuilder.Entity<BingoGame>().HasMany(bg => bg.Players).WithMany(p => p.BingoGames);

        modelBuilder.Entity<Leaderboard>().Navigation(l => l.Positions).AutoInclude();

        modelBuilder.Entity<LeaderboardPosition>().Navigation(lp => lp.Player).AutoInclude();
    }
}
